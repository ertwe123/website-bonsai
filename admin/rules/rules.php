...
<head>
  ...
  <title>Sistem Pakar Tanaman Bonsai</title>
...
<body>
  ...
  <header id="header" class="header fixed-top d-flex align-items-center">
    ...
    <div class="d-flex align-items-center justify-content-between">
      <a href="../index.html" class="logo d-flex align-items-center">
        ...
        <span class="d-none d-lg-block">Tanaman Bonsai</span>
      </a>
      <i class="bi bi-list toggle-sidebar-btn"></i>
    </div><!-- End Logo -->
    <div class="container">
      <marquee>Selamat Datang Diaplikasi Sistem Pakar Penyakit Tanaman Bonsai</marquee>
    </div>
  </header><!-- End Header -->
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link" href="../rules/rules.php">
          ...
          <span>Rule Certainty Factor</span>
        </a>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Rule Certainty Factor</h1>
      <nav style="--bs-breadcrumb-divider: '>';">
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="../index.php">Admin</a></li>
          <li class="breadcrumb-item active">Rule Certainty Factor</li>
        </ol>
      </nav>
      <hr>
    </div><!-- End Page Title -->
          ...
          <div class="table-responsive">
            ...
            <div class="konten">
              <form id="form1" name="form1" method="post" action="./simpanrules.php" enctype="multipart/form-data" onsubmit="return validateForm()">
                <h5 class="text-center">Tambah Rule Penyakit Penyakit Tanaman Bonsai</h5>
...
// =======================
$arrGejala = [];
$qGejala = mysqli_query($koneksi, "SELECT * FROM tb_gejala ORDER BY id");
while ($g = mysqli_fetch_assoc($qGejala)) {
    $arrGejala[$g['id']] = $g['kdgejala'] . " | " . $g['gejala'];
}

// =======================
// Ambil penyakit VALID (JOIN)
// =======================
$penyakit_valid = [];
$qPenyakit = mysqli_query($koneksi, "
    SELECT DISTINCT r.id_penyakit, p.nama_penyakit
    FROM tb_rules r
    JOIN tb_penyakit p ON r.id_penyakit = p.id
");

while ($p = mysqli_fetch_assoc($qPenyakit)) {
    $penyakit_valid[$p['id_penyakit']] = $p['nama_penyakit'];
}
?>
...
<table class="table table-striped table-hover table-bordered mt-4">
    <thead>
        <tr>
            ...
            <?php foreach ($penyakit_valid as $idp => $nama): ?>
                <th class="text-center">
                    <?= $nama ?>
                    <br>
                    <a href="./editrules.php?kdpenyakit=<?= $idp ?>">Edit Rule</a>
                </th>
            <?php endforeach; ?>
    ...
    <tbody>
        <?php
        $qGejalaRule = mysqli_query($koneksi, "
            SELECT DISTINCT id_gejala 
            FROM tb_rules 
        ...
        while ($row = mysqli_fetch_assoc($qGejalaRule)) {
            $idG = $row['id_gejala'];
        ?>
        <tr>
            ...
            <td>
                <?= isset($arrGejala[$idG]) ? $arrGejala[$idG] : '-' ?>
            </td>
            ...
            <?php foreach ($penyakit_valid as $idp => $nama): ?>
                <td class="text-center">
                    ...
                    if ($data) {
                        echo "<strong>
                            <a href='./editbelief.php?id_penyakit=$idp&id_gejala=$idG&belief={$data['belief']}'>
                                {$data['belief']}
                            </a>
                        </strong>";
                    } else {
